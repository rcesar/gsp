...
var stateInscriptionValidator = [
    validate({
        ...
        message: 'Inscrição estadual aceita somente números. O tamanho deve ser entre 5 e 20 números.'
    })
];
...
var municipalInscriptionValidator = [
    validate({
        validator: 'matches',
        arguments: ['^[0-9]{5,20}$', 'i'],
        message: 'Inscrição municipal aceita somente números. O tamanho deve ser entre 5 e 20 números.'
    })
];
    ...
    CustomersModel.findOne({
        ...
        if (err) {
            ...
            if (customer && customer.customerName) {
                next(errorUtils.getValidationError(prop.config.http.bad_request, i18n.__('validation').customers_save_cnpj_customername_duplicated));
            } else {
                next();
            }
        }
